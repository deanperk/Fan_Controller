/*
Fan Controller Code Enclosure Version 0.1B
 ...
 *----------------------------------------------------------- */                  
 ...
 const int remoteLED = 10; //amber override LED
 ...
 volatile boolean remoteContact = false; //fan remote contact -  close to run fan
...
void remoteContactRise() {
   operatingState = 4; //operating state remote contact asserted
...
attachInterrupt (digitalPinToInterrupt (3),remoteContactRise,RISING);
...
if(batteryType == HIGH) {
  lowVolt = 11.5;
  digitalWrite(twelveLED, HIGH); //turn on 12V LED
  digitalWrite(twentyfourLED, LOW);
}
if(batteryType == LOW) {
  lowVolt = 23.0;
  digitalWrite(twentyfourLED, HIGH);//turn on 24V LED 
  digitalWrite(twelveLED, LOW);
}
...
Serial.print("remote contact = ");//for debug
Serial.println(remoteContact); //for debug
Serial.print("Battery Type = "); //for debug
Serial.println(batteryType); //for debug
Serial.print("Low Voltage = "); //for debug
Serial.println(lowVolt); //for debug
Serial.println();
startTime = millis(); //set startTime for delay loop
  while ((startTime + delay200 + delay200) > millis() ) {
   } //200 mS delay
