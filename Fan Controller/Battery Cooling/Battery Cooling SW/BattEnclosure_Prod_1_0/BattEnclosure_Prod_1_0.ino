 /*
Fan Controller Code Battery Version Prod 1.0
...
Rev History
0.1 Proto 1
0.2 Adds delta temperature between amb and batt to determine Operating State
0.3 Changes POST to 3 sets of red LED Flashes
1.0 Removed serial print debug 
 ...
 *----------------------------------------------------------- */                  
 ...
 unsigned long delay200 = 500; //delay 200ms 
...
if (battVolt < lowVolt) {
  ...
  if ((ambTemperature > 200.0) || (ambTemperature < -20.0) || (battTemperature > 200.0) || (battTemperature < -20.0)) {
    ...
    if ((battTemperature < 75.0) && (battTemperature >= 50.0) && (digitalRead(remoteContact)== LOW)) {
            operatingState = 2; //OK Mode
            } else {  
             if ((battTemperature >= 75.0) && (battTemperature - ambTemperature >= 5.0) && (digitalRead(remoteContact)== LOW)) {
                operatingState = 3;//cool mode
            }  else {
              if ((battTemperature >= 75.0) && (battTemperature - ambTemperature < 5.0) && (digitalRead(remoteContact)== LOW)) {
                 operatingState = 5;//cooling desired mode
              } else {
              	if ((battTemperature < 50.0) && (ambTemperature - battTemperature >= 5.0) && (digitalRead(remoteContact)== LOW)) {
                 operatingState = 6;//heat mode
              } else {
                if ((battTemperature < 50.0) && (ambTemperature - battTemperature < 5.0) && (digitalRead(remoteContact)== LOW)) {
                 operatingState = 7; }//heating desired mode}
...
while (operatingState == 4) { //remote contact closure
        //loop here 
}
if (operatingState == 0) { //low voltage 
  digitalWrite(coolLED, LOW);
  digitalWrite(fanLED, LOW); 
  digitalWrite(remoteLED, LOW);
  digitalWrite(okLED, LOW);
  ...
  if (operatingState == 1) { //Thermistor fault
    digitalWrite(okLED, LOW);
    digitalWrite(coolLED, LOW);
    digitalWrite(fanLED, LOW); 
    digitalWrite(fanControl, LOW);
  ...
  if (operatingState == 5) { //cooling desired
    digitalWrite(okLED, LOW);
    digitalWrite(remoteLED, LOW);
    digitalWrite(fanLED, LOW); 
    digitalWrite(fanControl, LOW);
  ...
  while ((startTime + delay200) > millis() ) {
   ...
   if (operatingState == 6) { //heating
    ...
    if (operatingState == 7) { //heating desired
    digitalWrite(remoteLED, LOW);
    digitalWrite(coolLED, LOW);
    digitalWrite(fanLED, LOW);
    digitalWrite(okLED, LOW);  
...
//Serial.print("Remote Contact State =  ");
//Serial.print(remoteContactState);
//Serial.println();
//startTime = millis(); //set startTime for delay loop
